...
(function($, window) {
  ...
  $.formUtils.addValidator({
    ...
    validatorFunction: function(value, $el, config, language, $form) {
      var password,
        passwordInputName =
          $el.valAttr("confirm") || $el.attr("name") + "_confirmation",
        $passwordInput = $form.find('[name="' + passwordInputName + '"]');
      if (!$passwordInput.length) {
        $.formUtils.warn(
          "Password confirmation validator: could not find an input " +
            'with name "' +
            passwordInputName +
            '"',
          true
      ...
      if (allowing.length > 0) {
        ...
        $.each(allowing, function(i, cardName) {
          if (cardName in creditCards) {
            if (
              value.length >= creditCards[cardName][0] &&
            ...
            $.formUtils.warn(
              'Use of unknown credit card "' + cardName + '"',
              true
            );
          }
        });
  ...
  $.formUtils.addValidator({
    ...
    validatorFunction: function(val, $el) {
      ...
      return (
        $.formUtils.validators.validate_strength.calculatePasswordStrength(
          val
        ) >= requiredStrength
      );
    },
    ...
    strengthDisplay: function($el, options) {
      ...
      $el.bind("keyup", function() {
        var val = $(this).val(),
          $parent =
            typeof config.parent === "undefined"
              ? $(this).parent()
              : $(config.parent),
          $displayContainer = $parent.find(".strength-meter"),
          strength = $.formUtils.validators.validate_strength.calculatePasswordStrength(
            val
          ),
          css = {
            background: "pink",
  ...
  var requestServer = function(serverURL, $element, val, conf, callback) {
    var reqParams =
        $element.valAttr("req-params") ||
        $element.data("validation-req-params") ||
        {},
    ...
    $.ajax({
      ...
      error: function(error) {
        handleResponse(
          {
            valid: false,
            message: "Connection failed with status: " + error.statusText
          },
          callback
        );
        return false;
  ...
  $.formUtils.addAsyncValidator({
    ...
    validatorFunction: function(done, val, $input, conf, lang, $form) {
      var serverURL =
        $input.valAttr("url") || conf.backendUrl || document.location.href;
  ...
  $.formUtils.addValidator({
    ...
    validatorFunction: function(value, $input, config, language) {
      var numRequiredUppercaseChars =
          $input.valAttr("require-uc-letter") || "0",
      ...
      $.each(subValidators, function(name, subValidator) {
        ...
        if (numRequired) {
          ...
          if (!valid) {
            ...
            errorMessage +=
              language.passwordComplexitySeparator +
              numRequired +
              subValidator.dialogEnd;
