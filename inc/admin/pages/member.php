<div class="wrap container-fluid">
    <div class="row justify-content-between">
        ...
        <div class="col-auto">
            <a href="<?=wp_nonce_url(admin_url("users.php?action=delete&user=$uid"),'bulk-users')?>" class="btn btn-danger mb-1">حذف کاربری</a>
            <?php if($user->get('status') != 'proved'):?>
            <a href="<?=wp_nonce_url(admin_url("users.php?page=member&mid=$uid"),'prove_member')?>" class="btn btn-success mb-1">تایید کاربری</a>
            <?php elseif($user->get('status') == 'proved'):?>
            <a href="<?=wp_nonce_url(admin_url("users.php?page=member&mid=$uid"),'inactive_member')?>" class="btn btn-warning mb-1">تعلیق کاربری</a>
            <?php endif?>
        </div>
        <?php endif?>
    </div>
